...
def parse_args(args):
    ...
    parser.add_option('-i', action="store", type="string", dest="input_file", help="Path/name of the map file",
                      default="input_level.txt")
    parser.add_option('-o', action="store", type="string", dest="output_file", help="Number of maps to be generated",
                      default="output_level.png")
...
def render_structure(level, output_path):
    ...
    for i in range(level_height):
        for j in range(level_width):
            ...
            if level[i][j] in escape:
                tile = tile_dict["connecting"]
            else:
                try:
